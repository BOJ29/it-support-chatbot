...
def chat():
    try:
        ...
        return jsonify({'message': f'❌ Error: {str(e)}', 'type': 'error'}), 500
...
def resolve_ticket(ticket_id):
    try:
        ...
        conn = sqlite3.connect(db_path)
        cursor = conn.cursor()
        cursor.execute('UPDATE tickets SET status = ? WHERE id = ?', ('Resolved', ticket_id))
        conn.commit()
        conn.close()
...
def send_message(ticket_id):
    try:
        ...
        conn = sqlite3.connect(db_path)
        cursor = conn.cursor()
        cursor.execute('INSERT INTO messages (ticket_id, sender, message) VALUES (?, ?, ?)', 
...
def get_bot_stats():
    try:
        ...
        cursor.execute("""
            ...
            FROM bot_sessions WHERE outcome = 'Resolved by Bot'
            GROUP BY staff_email ORDER BY chat_count DESC
        """)
        bot_users = [dict(row) for row in cursor.fetchall()]
        
        cursor.execute("""
            SELECT staff_name, staff_email, COUNT(*) as ticket_count 
            FROM tickets GROUP BY staff_email ORDER BY ticket_count DESC
        """)
        escalated_users = [dict(row) for row in cursor.fetchall()]
        ...
        return jsonify({
            ...
            'bot_users': bot_users,
            'escalated_users': escalated_users
        })
    except Exception as e:
...
def export_tickets():
    try:
        ...
        writer.writerow(['Ticket ID', 'Staff Name', 'Staff Email', 'Issue', 'Priority', 'Status', 'Created Date'])
        
        for ticket in tickets:
            writer.writerow([
                ticket.get('id', ''), ticket.get('staff_name', ''), ticket.get('staff_email', ''),
                ticket.get('issue_description', ''), ticket.get('priority', ''), ticket.get('status', ''),
...
def get_stats():
    try:
        ...
        cursor.execute("""
            ...
            FROM tickets WHERE created_date LIKE ?
        """, (current_month + '%',))
        ...
        cursor.execute("""
            ...
            FROM tickets WHERE created_date LIKE ? GROUP BY category
        """, (current_month + '%',))
        categories = [dict(row) for row in cursor.fetchall()]
        conn.close()
        
        return jsonify({'month': current_month, 'month_stats': month_stats, 'categories': categories})
    except Exception as e:
        print(f"Stats error: {e}")
        return jsonify({'error': str(e)}), 500
...
def test_email():
    try:
        ...
        result = sg.send_email("agmasiltd@gmail.com", "Test", "Test email")
